...
| 情報取得 | CPU / GPU / メモリ / マザーボード(I/O) / ストレージ / ネットワーク / バッテリーのセンサー値、ディスク SMART、OS・プロセス・ドライブ等。**`ICollector` 実装の追加で収集源を拡張可能** |
...
PcAgent.Diagnostics   (classlib)  … 情報取得 + ルール判定（決定的・LLM非依存）
  ...
  Collectors/    ICollector と実装（HardwareCollectors / SmartCollector / SystemCollector）… 拡張ポイント
...
- **収集の拡張**: `ICollector`（`Name` / `CollectAsync(...)`）を実装し DI 登録するだけで、新しい情報源（WiFi、温度センサー追加、イベントログ等）を足せる。ツール / `@`プロバイダー / `info` コマンドは登録済みコレクタを列挙して動的に対応する。
...
| （将来）WiFi | SSID / 信号強度 / リンク速度 / バンド / チャネル | Native WiFi API（`ICollector` 追加で対応） |
