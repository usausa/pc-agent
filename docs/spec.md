...
> **ステータス**: 実装完了（P0–P10 ＋任意拡張・主要経路を実機検証済み）／最終更新 2026-06-27
...
> 本書を読めば現状と次の一手が分かるようにするための要約。詳細は各章を参照。

- **完了**: 計画フェーズ **P0–P10 ＋任意拡張**をすべて実装し、主要経路を実機検証済み。ビルドは**警告 0 / エラー 0**。必須の残作業はない。
  - 実装済みの任意拡張: メトリクス・Aspire 受信（§9）/ 評価 `LocalEvaluator`（§6.2）/ 履歴圧縮（§6.2）/ LLM シェル（§5.3）/ **Wi-Fi・イベントログ コレクタ**（§5.1）/ スラッシュ `/watch`・`/copy`・`/save`。
- **実機未検証の経路**（コードはビルド通過・縮退動作は確認済み。環境依存で未確認）:
  - **Wi-Fi の接続中表示** … 検証機に WiFi アダプタが無いため。WiFi 搭載機で `info wifi` / `@wifi` で確認する。
  - **`/watch` のライブ更新** … 非対話（出力リダイレクト）環境のため。対話起動で `/watch cpu` で確認する（一過性 UI の方針は §8.2）。
- **次の作業候補（任意・必須ではない）**: 内容面の評価（数値妥当性・診断的確さの決定的照合）。方針は [§6.2](#62-任意機能の判断)。なお **LLM-as-judge / マルチエージェント / 構造化出力は不採用**（§6.2）。
