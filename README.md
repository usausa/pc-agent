...
- **情報取得**: ハードウェア(CPU/GPU/メモリ/ディスク)・SMART・**Wi-Fi**・**イベントログ**・システム情報の収集（`ICollector` で拡張可能）。
...
| コマンド | 説明 |
| --- | --- |
| `info <category>` | 指定カテゴリ(cpu/gpu/memory/disk/smart/battery/wifi/eventlog/system)の情報を表示 |
...
| --- | --- |
| `/help` | コマンド一覧を表示 |
| `/info <category>` | PC 情報を表示（cpu/gpu/memory/disk/smart/battery/wifi/eventlog/system） |
...
| `/watch <category>` | メトリクスをライブ監視（キーで終了） |
| `/copy` | 直近のエージェント応答をクリップボードにコピー |
| `/save [path]` | 直近のエージェント応答をファイルに保存 |
| `/clear` | 画面をクリア |
| `/exit` | 終了（セッション概要を表示） |
...
詳細な設計・アーキテクチャは [`docs/spec.md`](docs/spec.md) を参照。
